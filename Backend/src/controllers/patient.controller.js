  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        patient,
        "Patient record  successfully. but not medical reports"
      )
    );
});
...
const updateMedicalHistory = asyncHandler(async (req, res) => {
  const { medicalHistory,userId } = req.body; // Assuming userId is passed in the request object
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, patient, "Medical history updated successfully.")
    );
});
...
const addMedicalReports = asyncHandler(async (req, res) => {
  ...
  const {userId} = req.body; // Assuming userId is passed in the request object
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, patient, "Medical reports added successfully."));
});
